...
import com.quandev.demo.entities.Cart;
...
public class HomeDaoIml implements HomeDao {
  ...
  public List<Menus> getParentsMenu(int id) {
    Session session = sessionFactory.openSession();
    Query query = session.createQuery("from Menus where parents=?1");
    query.setParameter(1, id);
  ...
  public List<Posts> getCollectionBlog(int id) {
    Session session = sessionFactory.openSession();
    Query query = session.createQuery("from Posts where categoryId = ?1 and status = 1 order by createTime desc");
    query.setParameter(1, id);
